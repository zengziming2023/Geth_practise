...
// ExchangeTokensPurchasedIterator is returned from FilterTokensPurchased and is used to iterate over the raw logs and unpacked data for TokensPurchased events raised by the Exchange contract.
type ExchangeTokensPurchasedIterator struct {
	Event *ExchangeTokensPurchased // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *ExchangeTokensPurchasedIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(ExchangeTokensPurchased)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *ExchangeTokensPurchasedIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *ExchangeTokensPurchasedIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// ExchangeTokensPurchased represents a TokensPurchased event raised by the Exchange contract.
type ExchangeTokensPurchased struct {
	User          common.Address
	Amount        *big.Int
	Price         *big.Int
...
// FilterTokensPurchased is a free log retrieval operation binding the contract event 0xf4720a91274d131c55ac5bd98a14c1ad24e0d0cadcfa35e1aec01b1c35cd8826.
//
// Solidity: event TokensPurchased(address indexed user, uint256 indexed amount, uint256 indexed price, address purchaseToken, uint256 lockId)
func (_Exchange *ExchangeFilterer) FilterTokensPurchased(opts *bind.FilterOpts, user []common.Address, amount []*big.Int, price []*big.Int) (*ExchangeTokensPurchasedIterator, error) {
	...
	logs, sub, err := _Exchange.contract.FilterLogs(opts, "TokensPurchased", userRule, amountRule, priceRule)
	if err != nil {
		return nil, err
	}
	return &ExchangeTokensPurchasedIterator{contract: _Exchange.contract, event: "TokensPurchased", logs: logs, sub: sub}, nil
}

// WatchTokensPurchased is a free log subscription operation binding the contract event 0xf4720a91274d131c55ac5bd98a14c1ad24e0d0cadcfa35e1aec01b1c35cd8826.
//
// Solidity: event TokensPurchased(address indexed user, uint256 indexed amount, uint256 indexed price, address purchaseToken, uint256 lockId)
func (_Exchange *ExchangeFilterer) WatchTokensPurchased(opts *bind.WatchOpts, sink chan<- *ExchangeTokensPurchased, user []common.Address, amount []*big.Int, price []*big.Int) (event.Subscription, error) {
	...
	logs, sub, err := _Exchange.contract.WatchLogs(opts, "TokensPurchased", userRule, amountRule, priceRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(ExchangeTokensPurchased)
				if err := _Exchange.contract.UnpackLog(event, "TokensPurchased", log); err != nil {
					return err
				}
				event.Raw = log
...
// ParseTokensPurchased is a log parse operation binding the contract event 0xf4720a91274d131c55ac5bd98a14c1ad24e0d0cadcfa35e1aec01b1c35cd8826.
//
// Solidity: event TokensPurchased(address indexed user, uint256 indexed amount, uint256 indexed price, address purchaseToken, uint256 lockId)
func (_Exchange *ExchangeFilterer) ParseTokensPurchased(log types.Log) (*ExchangeTokensPurchased, error) {
	event := new(ExchangeTokensPurchased)
	if err := _Exchange.contract.UnpackLog(event, "TokensPurchased", log); err != nil {
		return nil, err
	}
